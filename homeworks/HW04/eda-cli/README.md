...
```bash
uv run eda-api eda_cli.api:app --reload --port 8080
```
...
### 4. `POST /quality-from-parquet` – оценка качества по parquet-файлу

Эндпоинт принимает parquet-файл, внутри:
...
- в `/docs` открыть `POST /quality-from-parquet`,
- нажать `Try it out`,
- выбрать файл (например, `data/example.parquet`),
- нажать `Execute`.

**Пример вызова через `curl` (Linux/macOS/WSL):**

```bash
curl -X POST "http://127.0.0.1:8000/quality-from-parquet" \
  -F "file=@data/example.parquet"
```
...
- `ok_for_model` - результат по эвристикам;
- `quality_score` - интегральный скор качества;
- `flags` - все флаги из `compute_quality_flags`;
- `dataset_shape` - реальные размеры датасета (`n_rows`, `n_cols`);
- `latency_ms` - время обработки запроса.
...
Логирование в проекте реализовано с помощью `loguru`.
