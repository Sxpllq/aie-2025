...
typer_app = typer.Typer(help="Uvicorn runner for API")
...
class QualityResponse(BaseModel):
    ...
    flags: dict[str, bool | str | int | float] | None = Field(
        default=None,
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    logger_file = logger.bind(file_name=file.filename)
    ...
    logger_file.debug(file.filename)
    logger_file.debug(asdict(summary))
    logger_file.debug(f"Latency in ms is {latency_ms:.1f}")
...
@app.post(
    "/quality-from-parquet",
    response_model=QualityResponse,
    tags=["quality"],
    summary="Оценка качества по Parquet-файлу с использованием EDA-ядра",
)
async def quality_from_parquet(file: UploadFile = File(...)) -> QualityResponse:
    """
    Эндпоинт, который принимает Parquet-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает оценку качества данных.
    ...
    if file.content_type not in ("application/parquet", "application/octet-stream"):
        # content_type от браузера может быть разным, поэтому проверка мягкая
        # но для демонстрации оставим простую ветку 400
        raise HTTPException(status_code=400, detail="Ожидается Parquet-файл (content-type application/parquet).")

    try:
        # FastAPI даёт file.file как file-like объект, который можно читать pandas'ом
        df = pd.read_parquet(file.file)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=400, detail=f"Не удалось прочитать Parquet: {exc}")

    if df.empty:
        raise HTTPException(status_code=400, detail="Parquet-файл не содержит данных (пустой DataFrame).")
    
    filename = file.filename
    logger_file = logger.bind(filename=filename)
    ...
    if ok_for_model:
        message = "Parquet выглядит достаточно качественным для обучения модели (по текущим эвристикам)."
    else:
        message = "Parquet требует доработки перед обучением модели (по текущим эвристикам)."
    ...
    logger_file.info(file.filename)
    logger_file.debug(asdict(summary))
    logger_file.debug(f"Latency in ms is {latency_ms:.1f}")
...
@typer_app.command()
def run_api(
    app: str = typer.Argument("eda_cli.api:app", help="The ASGI app to run"),
    host: str = typer.Option("0.0.0.0", help="Host to run the API on"),
    port: int = typer.Option(8000, help="Port to run the API on"),
    reload: bool = typer.Option(False, help="Enable auto-reload for development"),
    log_level: str = typer.Option("info", help="Logging level for the API")
):
