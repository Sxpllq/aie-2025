...
def compute_quality_flags(
    ...
    flags["max_missing_share"] = round(max_missing_share, 3)
    flags["too_many_missing"] = max_missing_share > min_missing_share

    # флаг константных колонок
    n_constant_cols = sum(1 for col in summary.columns if col.unique <= 1)
    flags["n_constant_columns"] = n_constant_cols

    # высокая кардиальность категориальных колонок
    n_high_cardinality = sum(
        1
        for col in summary.columns
        if not col.is_numeric and col.unique > 0.5 * summary.n_rows
    )
    flags["n_high_cardinality"] = n_high_cardinality
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if n_constant_cols > 0:
        score -= 0.1
    if n_high_cardinality > 0:
        score -= 0.1
